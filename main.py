...
#root.iconbitmap('accessories_text_editor_16842.ico')
...
text = Frame(root)
text.pack(fill=BOTH, expand=1)  # create window
...
text.pack(expand=1, fill=BOTH, side=LEFT)  # open window (color\text all length)

# SCROLLBAR


scroll = Scrollbar(text, command=text.yview)  # scrollbar
scroll.pack(side=RIGHT, fill=Y)  # add scrollbar right
text.config(yscrollcommand=scroll.set)  # application to the form
